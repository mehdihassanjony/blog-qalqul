...
export class ArticleService {
  ...
  async getArticlesCountByCategory(category: CategoryEntity): Promise<number> {
    const articlesCount = await this.articleRepo
      ...
      .innerJoin('article.categories', 'category', 'category.id = :catId', {
        catId: category.id,
      })
      .getCount();
  ...
  async createArticle(
    ...
    articleToCreate.author = await this.userService.getOneUserByEmail(
      userEmail,
    );
    articleToCreate.categories = await this.categoryIdsToEntities(
      articleDto.categoryIds,
    );
  ...
  async updateArticle(
    ...
    await this.articleRepo.findOneOrFail({ where: { id: articleId } });
    ...
    const articleUpdated = await this.articleRepo.findOneOrFail({
      where: { id: articleId },
    });
  ...
  // private
  private async categoryIdsToEntities(
    catIds: number[],
